...
let phones = new Array("Photon 6E", "Photon 6X", "Photon 7E", "Photon 7X", "Photon 8P");
...
for (let i = 0; i < phones.length; i++) {
   ...
   for (let j = 0; j <= barPercent; j++) {
    ...
    //  tbody.insertAdjacentHTML('beforebegin', 'test');

   // Insert the bChart HTML code into first tbody element in the web document
   //const cuerpo = document.querySelector("cuerpo"); //.insertAdjacentHTML("beforebegin", barChart);
   //cuerpo.insertAdjacentHTML('beforebegin', barChart);

   window.addEventListener('load', () => {
      const tbody = document.querySelector('tbody');
      tbody.insertAdjacentHTML('beforebegin', barChart);
   });
